...
// Theme functions.
// =============================================================================
define("SIGNFLOW_URL", get_template_directory_uri());
define("SIGNFLOW_PATH", get_template_directory());
require_once(SIGNFLOW_PATH . "/inc/class-tgm-plugin-activation.php");
add_theme_support('post-thumbnails');
function signflow_register_required_plugins() {
	$plugins = array(
        array(
			'name'               => esc_html__('Hybrid Composer',"signflow"),
			'slug'               => 'hybrid-composer',
			'source'             => SIGNFLOW_PATH . '/inc/hybrid-composer.zip',
			'required'           => true,
			'version'            => '',
...
add_action( 'tgmpa_register', 'signflow_register_required_plugins' );
if (!isset( $content_width )) $content_width = 1200;
add_theme_support( 'automatic-feed-links' );
add_theme_support('title-tag');
function signflow_enqueue_front_end_script() {
    if (!defined("HC_PLUGIN_PATH")) {
        wp_enqueue_style("bootstrap", SIGNFLOW_URL . "/css/bootstrap/css/bootstrap.css", array(), "1.0", "all");
        wp_enqueue_script("bootstrap",  SIGNFLOW_URL . '/css/bootstrap/js/bootstrap.min.js', array("jquery"), "1.0", true);
        wp_enqueue_script("hc_script",  SIGNFLOW_URL . '/inc/default.js', array(), "1.0",true);
        wp_enqueue_style("hc_style", SIGNFLOW_URL . "/style.css", array(), "1.0", "all");
        wp_enqueue_style("hc_icons", SIGNFLOW_URL . "/inc/icons/icons.css", array(), "1.0", "all");
        wp_enqueue_style("hc_css_skin", SIGNFLOW_URL . "/skin.css", array(), "1.0", "all");
    }
    if (is_singular() && comments_open() && (get_option('thread_comments') == 1)) wp_enqueue_script('comment-reply');
}
add_action('wp_enqueue_scripts', 'signflow_enqueue_front_end_script');
...
<select data-hc-setting="sidebars" id="sidebars-menu" name="sidebars-menu">
    <option value="" <?php if ($sidebar == "") echo "selected" ?>><?php esc_attr_e("None","signflow") ?></option>
    <option value="right" <?php if ($sidebar == "right") echo "selected" ?>><?php esc_attr_e("Right","signflow") ?></option>
    <option value="left" <?php if ($sidebar == "left") echo "selected" ?>><?php esc_attr_e("Left","signflow") ?></option>
    <option value="right-left" <?php if ($sidebar == "right-left") echo "selected" ?>><?php esc_attr_e("Right and left","signflow") ?></option>
</select>
<?php
...
add_action('add_meta_boxes', 'signflow_sidebar');

function signflow_theme_add_editor_styles() {
    add_editor_style(SIGNFLOW_URL . "/css/tiny-mce.css");
}
add_action('admin_init', 'signflow_theme_add_editor_styles');

//MENU
function signflow_init_menus() {
    register_nav_menus(
          array(
            'header-menu' => esc_html__('Header Menu',"signflow"),
            'footer-menu' => esc_html__('Footer Menu',"signflow"),
            'extra-menu' => esc_html__('Top mini Menu',"signflow")
          )
    );
}
add_action('after_setup_theme', 'signflow_init_menus');

//WIDGETS
function signflow_widgets_init() {
    register_sidebar( array(
        'name'          => esc_html__('Right Sidebar',"signflow"),
        'id'            => 'right_side_bar',
        'description'   => esc_html__('Global sidebar for pages, enable it on single page.',"signflow"),
    ...
    register_sidebar(array(
        'name'          => esc_html__('Left Sidebar',"signflow"),
        'id'            => 'left_side_bar',
        'description'   => esc_html__('Global sidebar for pages, enable it on single page.',"signflow"),
        'before_widget' => '<div class="list-group list-blog">',
        'after_widget'  => '</div>',
    ...
    if (class_exists('WooCommerce')) {
        register_sidebar(array(
           'name'          => esc_html__('Woocommerce Shop Sidebar Left',"signflow"),
           'id'            => 'woocommerce_shop_left_side_bar',
           'description'   => esc_html__('Shop sidebar, enable it on Theme options > List Post Types',"signflow"),
           'before_widget' => '<div class="list-group list-blog">',
           'after_widget'  => '</div>',
           'before_title'  => '<p class="list-group-item active">',
           'after_title'   => '</p>',
        ...
        register_sidebar(array(
          'name'          => esc_html__('Woocommerce Shop Sidebar Right',"signflow"),
        ...
        register_sidebar(array(
          'name'          => esc_html__('Woocommerce Item Sidebar Left',"signflow"),
          'id'            => 'woocommerce_single_left_side_bar',
          'description'   => esc_html__('Single product sidebar, enable it on Theme options > List Post Types',"signflow"),
          'before_widget' => '<div class="list-group list-blog">',
          'after_widget'  => '</div>',
          'before_title'  => '<p class="list-group-item active">',
          'after_title'   => '</p>',
        ...
        register_sidebar(array(
           'name'          => esc_html__('Woocommerce Item Sidebar Right',"signflow"),
...
add_action( 'widgets_init', 'signflow_widgets_init' );

//MAIN CONTENT
function signflow_get_post_info() {
    $txt = "";
    ...
    if ( ! empty( $categories ) ) {
        foreach( $categories as $category ) {
            $output .= '<a href="' . esc_url( get_category_link( $category->term_id ) ) . '" alt="' . esc_attr( sprintf( __("View all posts in %s","signflow"), $category->name ) ) . '">' . esc_html( $category->name ) . '</a>' . $separator;
        }
    ...
    <div class="header-base search-results-header">
        <div class="container">
            <div class="row">
                <div class="col-md-9">
                    <div class="title-base text-left">
                        <h1><?php echo esc_html__("Search results for ","signflow") . get_search_query() ?></h1>
                        <p><?php esc_attr_e("Search results for ","signflow"); echo '<b><i>' . get_search_query() . '</i></b>'; ?></p>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="search-page-box">
                    <form role="search" method="get" id="searchform" class="navbar-form" action="<?php echo esc_url(home_url()) ?>">
                        <div class="input-group">
                            <input name="s" id="s" type="text" class="form-control" placeholder="<?php esc_attr_e("Search for ...","signflow"); ?>" />
                            <span class="input-group-btn">
                                <input class="btn btn-default" type="submit" id="searchsubmit" value="<?php esc_attr_e("Go","signflow"); ?>" />
                            </span>
                        </div>
                    </form>
                </div>
...
?>                         <h3>
            ...
            if (defined("HC_PLUGIN_PATH")) {
                echo hc_get_the_excerpt(get_the_excerpt());
                            ...
                            <a class="circle-button btn btn-sm" href="<?php echo esc_url($link) ?>">
                                <?php esc_attr_e("Read more","signflow") ?>
                            </a>
                        </div>
                    </div>
                </div>
    ...
    } else echo "<h2 class='no-search-results'>" . esc_html__("No results found ...","signflow") . "</h2><hr class='space row-500' />";
                ?>
            </div>
        </div>
    </div>
...
function signflow_set_default_menu() {
...
function signflow_default_blog() {
    global $wp_query;
    if (have_posts()) {
        while (have_posts()) {
            the_post(); 
   ...
   <div class="advs-box advs-box-side default-box <?php if (is_sticky()) echo "sticky-post"; ?>" data-anima="fade-left" data-trigger="hover">
    <div class="row">
        <div class="col-md-12">
            ...
            <?php echo signflow_get_post_info(); ?>
    ...
    <ul class="pagination-sm pagination-grid pagination default-pagination">
        <li class="prev">
            <?php echo get_previous_posts_link(__("< Previous","signflow")); ?>
        </li>
        <li class="next">
            <?php echo get_next_posts_link(__("Next >","signflow"), $wp_query->max_num_pages ); ?>
        </li>
    </ul>
    <?php }
}
